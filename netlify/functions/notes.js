...
export default async (req, context) => {
  ...
  // POST
  if (req.method === "POST") {
    const { theme_id, text } = await req.json();
    if (!theme_id || !text?.trim()) return error("theme_id y text requeridos");
    const [note] = await sql`
      INSERT INTO notes (theme_id, text, created_by)
      VALUES (${theme_id}, ${text.trim()}, ${user.id})
      RETURNING *
    `;
    await logActivity(user.id, "create_note", "note", note.id, { theme_id });
